...
class MemberIndexPage extends Component {
    ...
    renderHelp(){
        if(window.config.spec =="dafa-bt6" || window.config.spec== "dafa-ldl"){
            return <a href="/help/helpcenter.html" target="_blank" className="guide btn btn-lg btn-danger">新手指南</a>
        }else if(window.config.spec =="xhtd-bt6" || window.config.spec =="xhtd-ldl"){
            return <a href="/agent.html?tab=Faq" target="_blank" className="guide btn btn-lg btn-danger">新手指南</a>
        }else if(window.config.spec =="vns-bt6" || window.config.spec =="vns-ldl"){
            return <a href="/help.html#commonQ" target="_blank" className="guide btn btn-lg btn-danger">新手指南</a>
        }else if(window.config.spec =="xpj-bt6" || window.config.spec =="xpj-ldl"){
            return <a href="/help.html#rule" target="_blank" className="guide btn btn-lg btn-danger">新手指南</a>
        }
    }
    render() {
                ...
                <div className="userInfo">
                    ...
                    <div className="info">
                        <ul>
                            ...
                            <li style={{"width":"22.7%"}}><label>等级:</label><span className="con">{this.props.user.userLevelName}</span></li>
                            {
                                this.props.user.verfyPhone?
                                    <li><label>手机:</label><span className="con">{this.props.user.phone}<a className="validate">验证通过</a></span></li> 
                                :
                                    <li><label>手机:</label><span className="con">{this.props.user.phone}<a className="validate" style={{background:"#cf271e"}} data-toggle="modal" data-target="#bindPhoneModal" >尚未验证</a></span></li>
                            }
                            <li><label>姓名:</label><span className="con">{this.props.user.realName}</span></li>
                            <li className="spec-li"><label>推广链接:</label><input  id="recommendCode" className="conInput" type="text" value={location.protocol+"//"+ location.hostname + "/register?channel=" +this.props.user.recommendCode} readOnly /><a onClick={this.copyCode.bind(this)} className="validate">复制</a></li>
