...
class FirstPage extends Component {
    ...
    renderRightCon() {
        ...
        if (this.props.user.token) {
            dom = [
                <Link to='/signIn' className="toSignIn" key={'userInfo'}>签到有礼</Link>
            ]
        } else {
            dom = [
    ...
    canPlay(game) {
        ...
        if (game.category) {
            if (game.GameId) {
                ...
                browserHistory.push('/allGame/' + game.PlatformIds + '&' + game.GameType + '&' + game.Name)
            }
            return false;
        } else {
                    ...
                    title:
                        <div className="WapCategores">
                            <span className="tabicon">
                                {/*<img src={_this.state.nowTabActive == index ?*/}
                                    {/*config.devImgUrl + item.ImageUrl2 :*/}
                                    {/*config.devImgUrl + item.ImageUrl} />*/}
                            </span>
    ...
    */
    ...
    renderContentList() {
        ...
        if (this.state.loading) {
            contentList.push(<div key={"goLoading"} className="dataLoading"><i className="icon-spinner icon-spin"></i>玩命加载中..
            </div>)
        } else if (!this.state.hasMore) {
            contentList.push(<div style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                height: 'auto',
                backgroundColor: '#fff'
            }} key={"hasMore"} style={{textAlign: 'center', color: '#888', lineHeight: '3rem'}}>我没有更多数据了! (ㄒoㄒ)~</div>)
        } else {
            this.state.contentList.forEach((item, index) => {
                ...
                contentList.push(
                    <div className="listGame" key={index} onClick={this.canPlay.bind(this, item)}>
                        {favoriteBtn}
                        <img className="image" src={config.devImgUrl + imgUrl}/>
        ...
        return (
            <div className="firstPage">
                ...
                >
                    ...
                    {/*公告*/}
                    <NoticeBar marqueeProps={{loop: true, style: {padding: '1px 7.5px 0'}}}>
                        公告: {notice}
                    </NoticeBar>
                    ...
                    <TabExample titleList={wapCategores} contentList={contentList}
                                changeFun={this.tabsChange.bind(this)} index={this.props.wapCategory.index}/>
                </PullToRefresh>
