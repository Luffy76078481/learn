...
import {isShowDownApp} from 'globalAction';
...
class FirstPage extends Component{
            ...
            new window.actions.ApiWapGameCategoriesAction(item.MarkIds).fly(resp=>{
                if(resp.StatusCode ==0){
                    for(var i = 0; i < resp.Data.length; i++) {
                        resp.Data[i].category = true;//添加category为的是说明返回的数据是属于分类的（点击后可能会跳转到allGame页面）
                    }
                    _this.setState({
    ...
    canPlay(game){
        ...
        if (game.category) {//是否要跳转到allGame的AG老虎机页面
            if(game.GameId){
    ...
    renderWapCategores(){
        ...
        if(this.props.user.token){
            categores.push(<a key={"tabsCateSelf"} className={"tabsCate "+this.state.selfGamesClass} onClick={_this.selfGames.bind(this)}>我的游戏</a>);
        }
        return categores;
    }
    renderContentList(){
        ...
        if(this.state.loading){
            ...
            this.state.contentList.forEach((item,index)=>{
                ...
                contentList.push(
                    <div className="listGame" key={index} onClick={
                        this.state.IntoShowGameIdentify&&item.GameIdentify&&item.GameIdentify.indexOf("2$X")!=-1?
                        this.IntoRoom.bind(this,item):
                        this.canPlay.bind(this,item) 
                        }>
                        {favoriteBtn}
...
const mapStateToProps = (state, ownProps) => (
    {
        ...
        wapAdsList:state.wapAdsList,// banner
        remoteSysConfs:state.remoteSysConfs,// 客服链接信息等
        wapCategory:state.wapCategory,// 主页游戏分类
