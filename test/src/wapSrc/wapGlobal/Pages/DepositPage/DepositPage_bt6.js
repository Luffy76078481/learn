...
class DepositPage extends Component{
    ...
    componentWillReceiveProps(nextProps){
        if((this.props.allOfflineAccount != nextProps.allOfflineAccount)) {
        ...
        // if(this.props.allOfflineAccount.length != nextProps.allOfflineAccount.length){
        // }
    }
    ...
    renderALZZpay(){//银行转账（过滤支付宝VIP）
        PayMethodList=[];
        for (var i = 0; i < this.props.allOfflineAccount.length; i++) {
            var p = this.props.allOfflineAccount[i];
            if(p.Bank.BankName.indexOf("支付宝")!=-1){//过滤支付宝
                PayMethodList.push(
                    {
                        label: p.Bank.BankName,
                        selfObj:p,
    ...
    tabChange(key){
        let type = key;
        let index = 0;  
        if(type == "ALZZ"){//如果是支付宝转账的话，需要过滤掉银行卡默认下标。
            for (var i = 0; i < this.props.allOfflineAccount.length; i++) {
                var p = this.props.allOfflineAccount[i];
                if(p.Bank.BankName.indexOf("支付宝")!=-1){//过滤支付宝
                    index=i
        ...
        this.setState({
            ...
            showOfflinePay:type=="YHZZ" || type=="ALZZ",
            OnlinePay:{
                BankName:"",
                onlinePayTrueList:[]
            },
    ...
    render(){
        ...
        }else if(this.state.payTab == "ALZZ"){
            this.renderALZZpay();//银行支付宝转账
        }
