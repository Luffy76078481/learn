...
class PersonCenterPage extends Component {
    ...
    componentDidMount(){
        new ApiAllSysConfigAction("allow_edit_pay_pwd").fly(resp => {
            if(resp.StatusCode == 0 && resp.Config.allow_edit_pay_pwd=="true"){
    ...
    render() {
        return (
            <div className=" w1000" style={{paddingBottom: "20px"}}>
                ...
                <div className="clearfix t20 ht-cunkuan deposit member-content">
                    <div className="t tc f14" style={{"textAlign":"center"}}>
                        <Link to="/person" className={this.props.location.pathname === '/person'?'iblock on':'iblock'}>个人资料</Link>
                        <Link to="/person_change_pwd" className={this.props.location.pathname === '/person_change_pwd'?'iblock on':'iblock'}>修改登录密码</Link>
