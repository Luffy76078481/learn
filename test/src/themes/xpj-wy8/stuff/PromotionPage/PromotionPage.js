...
import {ApiPromoApplyAction} from 'globalAction';
...
class PromotionPage extends Component {
    ...
    applyAction(promo){
        new ApiPromoApplyAction(promo.PreferentialKey,promo.IsRule).fly(resp=>{
                        ...
                        <div className="promotions-list-content BGcolor-third promotions-border">
                            <div className="promotions-list-img" onClick={this.onSelectPromo.bind(this,promo.Id)} style={{background: "url("+window.config.prdImgUrl + promo.Img + ") no-repeat", cursor:"pointer"}}>
                                <div className="promotions-list-title">
                                    <h3>{promo.Title}</h3>
                                    {window.config.spec == "xpj-bt6"?null:<span>发布时间：{promo.StartTime.split('T')[0]}</span>}
                                    {promo.CanApply && <button className="can-apply" onClick={this.applyAction.bind(this,promo)}>申请优惠</button>}
                                </div>
                            </div>
