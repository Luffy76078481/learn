...
class DepositPage extends Component {
    ...
    howPay(){
        ...
        }else if(this.state.payType=="YHZZ" || this.state.payType=="ALZZ"){//银行转账
        ...
        if(this.props.allOfflineBanks.length>0){
            for (let i = 0; i < this.props.allOfflineBanks.length; i++) {
                ...
                if(this.state.offlineBankCode == ""){
                    ...
                    this.state.isPayLink = BankInfo.Type != 3 ;  // 是否是二维码
                }
    ...
    //渲染【支付宝VIP(从银行转账过滤支付宝出来)】的渠道
    renderALZZpay_option() {
        var ret = [];
        if(this.props.allOfflineBanks.length>0){
            ...
            for (let i = 0; i < this.props.allOfflineBanks.length; i++) {
                var BankInfo = this.props.allOfflineBanks[i];
                BankInfo.index = i;
                if(BankInfo.Bank.BankName.indexOf("支付宝")!=-1){//过滤支付宝
                    if(this.state.offlineBankCode == ""){
                        this.state.offlineBankId = i;
                        this.state.offlineBankCode = this.props.allOfflineBanks[i].Bank.Id;
                        this.state.hideAtmBox = true;
                        this.state.isPayLink = BankInfo.Type != 3 ;  // 是否是二维码
                    }
                    ret.push(<option key={i} value={JSON.stringify(BankInfo)}>{BankInfo.Bank.BankName}</option>)
    ...
    renderTabsUl(){     
        ...
        for(let i=0;i<bankSys.length;i++){
            ...
            switch (item.key){
                case "ZXWY":
                    ret.push(//单独从银行转账过滤出支付宝转账
                        <div key='ALZZ' className={className} >
                            <div className="first">
                                <a  data-payIndex="" onClick={this.navOn.bind(this)} id="ALZZ">
                                支付宝VIP
    ...
    renderPayTypeDom(AmountSpan){
        ...
        }else if(this.state.payType=="ALZZ"){
            payTypeDom=(
                <div>
                    {this.renderALZZpay_option()}
                </div>
            )
        }else{
    ...
    render() {
        ...
        let AmountSpan;//限额提示
        let tips;
        if(this.state.payType=="YHZZ" || this.state.payType=="ALZZ"){//【银行转账】
            h2Tilte="银行卡存款";
                ...
                <div className="deposit">
                    ...
                    <div className="fromCon" hidden={!this.state.payType}>
                        ...
                        <div className="money" hidden={this.state.isLoading || this.state.thirdProgress}>
                            ...
                            {
                                (this.state.hideAtmBox && (this.state.payType=="YHZZ" || this.state.payType=="ALZZ")) ?
                                    (
                                        <div>
                                            <button onClick={()=>this.setState({payType:"",offlineBankCode:""})} className="btn goback">返回</button>
